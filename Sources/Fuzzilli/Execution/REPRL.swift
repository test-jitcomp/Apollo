...
public class REPRL: ComponentBase, ScriptRunner {
    // Generated by instrumented code for collecting coverage
    static let REPRL_COV_OUT_PREFIXES = "[COV] "
...
class REPRLExecution: Execution {
    ...
    var stdout: String {
        ...
        if cachedStdout == nil {
            let dirtyStdout = String(cString: reprl_fetch_stdout(reprl.reprlContext))
            let dirtyLines = dirtyStdout.split(separator: "\n", omittingEmptySubsequences: false)
            cachedStdout = dirtyLines[
                (dirtyLines.firstIndex(where: {
                    !$0.hasPrefix(REPRL.REPRL_COV_OUT_PREFIXES)
                }) ?? dirtyLines.startIndex)
                ..<
                dirtyLines.endIndex
            ].joined(separator: "\n")
        }
